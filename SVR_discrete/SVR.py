...
class DiscreteActor(nn.Module):
	def __init__(self, state_dim, action_dim):
		...
		self.l3 = nn.Linear(256, 256)  # output logits for each action
		# self.l3 = nn.Linear(256, 256)
		self.l4 = nn.Linear(256, action_dim)  # output logits for each action
	...
	def forward(self, state, deterministic=False):
		...
		if deterministic:
			...
			act = torch.multinomial(probs, num_samples=1).squeeze(-1) # sample
			# prob_act = probs.gather(1, act.unsqueeze(-1)).squeeze(-1)  # get the probability of the sampled action
		return act, logits
...
class SVR(object):
	def __init__(
		...
		self.device = device
		self.actor = DiscreteActor(state_dim, action_dim, max_action).to(self.device)
	...
	def train(self, batch_size=256, writer=None):
		...
		if self.total_it % 10000 == 0:
			with torch.no_grad():
				...
				unif = torch.distributions.uniform.Uniform(-1, 1).sample((batch_size, self.action_dim)).to(self.device)
