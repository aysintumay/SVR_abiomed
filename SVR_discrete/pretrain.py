...
parser.add_argument("--action_space_type", type=str, default="discrete", choices=["continuous", "discrete"], help="Type of action space for the environment") 
...
env, dataset, dataset_val = get_env_data(args, val=True)
...
if dataset_val is not None: #only for abiomed
    ...
    eval_states = replay_buffer_val.state
    eval_actions = replay_buffer_val.action
...
for step in tqdm(range(args.num_iters + 1), desc='train'):
    ...
    pi, logits = actor(train_states, deterministic= False)
    unnorm_pi = (env.world_model.unnorm_pl(train_actions).to(device) - 1).squeeze(1).long()
    loss = loss_fn(logits, unnorm_pi)
    ...
    if step % 100 == 0:
        ...
        if eval_states is not None and eval_actions is not None:
            ...
            with torch.no_grad():
                ...
                pi, logits = actor(eval_states_tensor, deterministic=True)
                loss = loss_fn(logits, (env.world_model.unnorm_pl(eval_actions_tensor).to(device) -1).squeeze(1).long())
    ...
    if step == args.num_iters:
        ...
        save_path = f"{args.save_path}SVR_discrete_bcmodels/bcmodel_{suffix}.pt"
