...
def load_data(data_path, test_size, validation_size, args=None):
    ...
    if args.temporal_split:
        ...
        X_train, X_test_val = train_test_split(
            X,
...
def plot_anomaly_results(X, y_true, y_pred, anomaly_scores, save_path=None):
    ...
    axes[0].scatter(
        X[normal_mask, 0], X[normal_mask, 1], c="blue", alpha=0.6, s=20, label="Normal"
    )
    axes[0].scatter(
        X[anomaly_mask, 0],
        X[anomaly_mask, 1],
    ...
    axes[1].scatter(
        X[pred_normal_mask, 0],
        X[pred_normal_mask, 1],
        c="blue",
        alpha=0.6,
        s=20,
    ...
    axes[1].scatter(
        X[pred_anomaly_mask, 0],
        X[pred_anomaly_mask, 1],
        c="red",
        alpha=0.8,
        s=20,
    ...
    scatter = axes[2].scatter(
        X[:, 0], X[:, 1], c=anomaly_scores, cmap="viridis", alpha=0.7, s=20
    )
...
def main():
    ...
    print(
        f"Test set anomalies detected: {anomaly_count}/{len(X_test)} ({anomaly_rate:.1%})"
    )
    print(f"Density score range: [{test_scores.min():.4f}, {test_scores.max():.4f}]")
    print(
        f"Anomaly score range: [{test_anomaly_scores.min():.4f}, {test_anomaly_scores.max():.4f}]"
    )
