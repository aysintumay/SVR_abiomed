...
def create_search_visualization(results_df, save_path):
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(save_path, "hyperparameter_search_results.png"),
        dpi=300,
        bbox_inches="tight",
    )
...
def run_hyperparameter_search(args, gamma_ranges):
    ...
    gamma_combinations = list(
        product(
            np.array(gamma_ranges["gamma1"]),
            np.array(gamma_ranges["gamma2"]),
            np.array(gamma_ranges["gamma3"]),
        )
    )
    ...
    for trial_id, (gamma1, gamma2, gamma3) in enumerate(gamma_combinations):
        try:
            ...
            print(
                f"Starting trial {trial_id} with gamma1={gamma1}, gamma2={gamma2}, gamma3={gamma3}"
            )
            policy, custom_metric, metrics, save_path = train(trial_args)
            if not os.path.exists(
                os.path.join(args.save_path, "SVR_hpo", save_path, t0)
            ):
            ...
            model_path = os.path.join(
                ...
                save_path,
                t0,
                f"svr_seed_{args.seed}_{trial_id}_gamma1_{gamma1}_gamma2_{gamma2}_gamma3_{gamma3}.pth",
            )
    ...
    print("=" * 80)
    print(
        f"Best Parameters: gamma1={best_params[0]:.4f}, gamma2={best_params[1]:.4f}, gamma3={best_params[2]:.4f}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="/abiomed/intermediate_data_d4rl/farama_sac_expert/Hopper-v2_expert_1000.pkl",
    )
    ...
    results_df, best_params, best_model_path = run_hyperparameter_search(
        args, gamma_ranges
    )
