...
from noisy_mujoco.abiomed_env.cost_func import (
    compute_acp_cost,
...
def eval_policy(
    ...
    if env_name == "abiomed":
        ...
        for k in range(eval_episodes):
            ...
            if wm is None:
                wm = (
                    eval_env.world_model
                )  # fallback to global `env` if that's how you access it
            # ACP (per-timestep across the episode)
            avg_acp += compute_acp_cost_model(
                wm, eval_env.episode_actions, ep_states_np
            )

            # AIR metrics
            total_map_air_sum += compute_map_model_air(
                wm, ep_states_np, eval_env.episode_actions
            )
            total_hr_air_sum += compute_hr_model_air(
                wm, ep_states_np, eval_env.episode_actions
            )
            total_pulsatility_air_sum += compute_pulsatility_model_air(
                wm, ep_states_np, eval_env.episode_actions
            )
            total_aggregate_air_sum += aggregate_air_model(
                wm, ep_states_np, eval_env.episode_actions
            )
            total_super_sum += super_metric(wm, ep_states_np, eval_env.episode_actions)

            # Weaning + unstable percentage
            wean_score_sum += weaning_score_model(
                wm, ep_states_np, eval_env.episode_actions
            )
            ...
            if (k == 100) and plot:
                fig = utils.plot_policy(
                    eval_env, next_state_l[1:], all_states, "SVR", writer, False
                )
        ...
        print("---------------------------------------")
        print(f"Evaluation over {eval_episodes} episodes: " f"Return {avg_reward:.3f}")
        print(f"ACP {acp_mean:.4f}")
        print(
            f"MAP AIR/ep: {map_air_mean:.5f} | HR AIR/ep: {hr_air_mean:.5f} "
        ...
        print("---------------------------------------")
        print(
            f"Evaluation over {eval_episodes} episodes: {avg_reward:.3f}, D4RL score: {avg_reward:.3f}"
        )
        print("---------------------------------------")
...
def eval_bc(
    ...
    if env_name == "abiomed":
        ...
        for k in range(eval_episodes):
            ...
            while not (done or truncated):
                s_norm = (np.array(state).reshape(1, -1) - mean) / std
                eval_states_tensor = torch.tensor(
                    s_norm, dtype=torch.float32, device=actor.device
                )
            ...
            if (k == 100) and plot:
                fig = utils.plot_policy(
                    eval_env, next_state_l[1:], all_states, "BC", writer
                )
            ...
            acp = compute_acp_cost_model(wm, eval_env.episode_actions, ep_states_np)
            acp_list.append(acp)
            avg_acp += acp

            total_map_air += compute_map_model_air(
                wm, ep_states_np, eval_env.episode_actions
            )
            total_hr_air += compute_hr_model_air(
                wm, ep_states_np, eval_env.episode_actions
            )
            total_puls_air += compute_pulsatility_model_air(
                wm, ep_states_np, eval_env.episode_actions
            )
            total_agg_air += aggregate_air_model(
                wm, ep_states_np, eval_env.episode_actions
            )
            ws = weaning_score_model(wm, ep_states_np, eval_env.episode_actions)
            ws_list.append(ws)
            total_wean_score += ws
            ...
            if (acp == 0.0) and (plotted_min_acp == 0):
                plotted_min_acp = 1
                utils.plot_policy(
                    eval_env, next_state_l[1:], all_states, "BC" + " Min ACP", writer
                )

            if (acp == 4.0) and (plotted_max_acp == 0):
                plotted_max_acp = 1
                utils.plot_policy(
                    eval_env, next_state_l[1:], all_states, "BC" + " Max ACP", writer
                )

            if ws < 0.0:
                ws_num += 1.0
            if ws == 0.0 and plotted_min_ws == 0:
                plotted_min_ws = 1
                utils.plot_policy(
                    eval_env, next_state_l[1:], all_states, "BC" + " Min WS", writer
                )
            if ws == 1.0 and plotted_max_ws == 0:
                plotted_max_ws = 1
                utils.plot_policy(
                    eval_env, next_state_l[1:], all_states, "BC" + "Max WS", writer
                )
        ...
        plt.show()
        fig.savefig(
            os.path.join(writer.log_dir, f"eval_samples_ACP.png"),
            dpi=300,
            bbox_inches="tight",
        )
        fig, ax1 = plt.subplots(figsize=(6, 4), dpi=300)
        ...
        plt.show()
        fig.savefig(
            os.path.join(writer.log_dir, f"eval_samples_WS.png"),
            dpi=300,
            bbox_inches="tight",
        )
        fig, ax1 = plt.subplots(figsize=(6, 4), dpi=300)
        ...
        plt.show()
        fig.savefig(
            os.path.join(writer.log_dir, f"eval_samples_RWD.png"),
            dpi=300,
            bbox_inches="tight",
        )
        ...
        print(
            f"  MAP AIR/ep: {map_air_avg:.5f} | HR AIR/ep: {hr_air_avg:.5f} | "
        ...
        print(
            "percentage of episodes with ACP >= 3.0:",
            (acp_num / eval_episodes) * 100.0,
            "maximum acp in eval episodes:",
            max(acp_list),
            "minimum acp in eval episodes:",
            min(acp_list),
        )
        print(
            "percentage of episodes with Weaning score <= 0.0:",
            (ws_num / eval_episodes) * 100.0,
            "maximum weaning score in eval episodes:",
            max(ws_list),
            "minimum weaning score in eval episodes:",
            min(ws_list),
        )
        print("---------------------------------------")
        return {
        ...
        for _ in range(eval_episodes):
            ...
            while not (done or truncated):
                ...
                action = actor(eval_states_tensor)
                state, reward, done, truncated, _ = eval_env.step(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--policy_path",
        type=str,
        default="/abiomed/models/policy_models/SVR_kde/abiomed/svr_kde_seed_1_2025-08-01_09-03-42_130000.pth",
    )
    ...
    parser.add_argument(
        ...
        default="bc",
        choices=["svr", "bc"],
        help="RL model to use for evaluation",
    )
    ...
    print(f"uploading {args.model_name_rl}")
